...
namespace ProjectMentorMatch.Models
{
    public class ProfileModels : Account
    {
        ...
        */
        ...
        {
            List<ProfileModels> mentors = new List<ProfileModels>();
        ...
        SELECT 
            ...
            p.MentorRate,
            p.MentorAboutMe
        FROM CreateAccount ca
        INNER JOIN Profile p ON ca.UserID = p.UserID
        WHERE p.IsMentor = 1"; // Adjust based on your logic to select mentors
                    ...
                    while (reader.Read())
                    {
                        mentors.Add(new ProfileModels
                        {
                         ...
                         //   Picture = reader["Picture"] as byte[],
                          ...
                          //  NonAcademic = reader["NonAcademic"].ToString(),
                         //   MentorAcad = reader["MentorAcad"].ToString(),
                         // //  MentorNonAcad = reader["MentorNonAcad"].ToString(),
                          //  MentorSched = reader["MentorSched"].ToString(),
                         //   Sched = reader["Sched"].ToString(),
                          //  MentorRate = reader["MentorRate"].ToString(),
                          //  MentorAboutMe = reader["MentorAboutMe"].ToString()
                        });
                    }
                }
            }
